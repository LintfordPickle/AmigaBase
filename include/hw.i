...
ExecBase       equ    $4                           ; Pointer to the Exec library base
 ...
 *
 * CIAA / CIAB
 * Handle the serial, parallel, keyboard and other IO activities
 *
CIAAPRA        equ    ciapra                       ; Input and output for pin 6 (port 1 and 2 fire buttons)

; 8 bit shift register connected to the keyboard. 
; It can work in two ways: in INPUT mode, the data generated by the keybaord is entered into the register. When all
; bits that make up the keystroke have been transmitted, a level 2 interrupt is generated.
; In OUTPUT mode, you write to the register. The swicthing between these two modes takes place via bit 6 of the CRA register.
CIAASDR        equ    ciasdr                       ; CIAA serial data register (connected to keyboard)
CIAAICR        equ    ciaicr                       ; Controls the interrupts that cna be generated by the CIAA                      
CIAACRA        equ    ciacra                       ; The control register. Bit 6 controls the SDR mode (input/output)
 *
 * EGA+
 *
BPLCON3        equ    $106
